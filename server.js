...
// ============================================================
// ESTADOS GLOBALES Y MAPAS
// ============================================================
const conexionesTikTok = {};
let participantes = {};
let subastaActiva = true; // Activo por defecto al conectar
...
function configurarEventosTikTok(tiktokConn, streamerId, io) {
    tiktokConn.on("gift", (data) => {
        try {
            ...
            // Filtro de racha de TikFinity
            if (data.giftType === 1 && data.repeatEnd === false) {
                return;
            }

            const userId = data.uniqueId;
            const giftName = data.giftName || "Regalo desconocido";
            ...
            if (mapValue) {
                ...
                diamantes = Number(data.totalDiamondCount) || (Number(data.diamondCount || 0) * repeatCount) || 0;
            }
            ...
            // Emitir actualización de ranking y eventos
...
// ============================================================
// SOCKET.IO (CONEXIÓN UNIFICADA SERVERTAP + TIKTOK)
// ============================================================
io.on("connection", (socket) => {
    console.log("🟢 Cliente conectado:", socket.id);

    socket.on("conectar-sistema", async (data) => {
        const { ip, port, password, playerName, tiktokUser } = data;
        const username = tiktokUser?.replace("@", "").trim();
        ...
        // Limpiar participantes al iniciar un nuevo sistema
        participantes = {};
        ...
        try {
            ...
            if (response.ok) {
                ...
                socket.emit("servertap-error", {
                    message: `Error HTTP ServerTap: ${response.status}. Revisa la contraseña.`
                });
                return;
            }
        } catch (error) {
            ...
            socket.emit("servertap-error", {
                message: "No se pudo alcanzar ServerTap. Revisa la IP, puerto o firewall."
            });
            return;
        }

        // 2. CONECTAR AL LIVE DE TIKTOK (Usando el método directo y limpio que sí funciona)
        console.log(`🎥 Conectando al Live de TikTok: @${username}`);
        socket.join(username);

        if (conexionesTikTok[username]) {
            try {
                if (typeof conexionesTikTok[username].disconnect === "function") {
                    conexionesTikTok[username].disconnect();
                } else if (typeof conexionesTikTok[username].stop === "function") {
                    conexionesTikTok[username].stop();
                }
            } catch (e) {}
    ...
    // Simulación de regalos desde el panel
    socket.on("simular-regalo", (data) => {
        const { user, amount } = data;
        const userId = "sim_" + (user || "TestUser");
        
        if (participantes[userId]) {
            participantes[userId].cantidad += (amount || 10);
        } else {
            participantes[userId] = {
                userId: userId,
